...
namespace Services
{
 public class BookingsGetterService : IBookingsGetterService
 {
  ...
  public virtual async Task<PaginatedResult<BookingsItemResult>> GetBookings(string? status, string? sortBy, string? sortDirection, int page, int pageSize)
  {
   ...
   {
    Items = paginatedResult.Items.Select(s => new BookingsItemResult(s.Id, s.Status, s.TotalPrice, s.NumNights, s.NumGuests, s.Guest.FullName, s.Guest.Email, s.Guest.CountryFlag, s.StartDate.ToString(), s.EndDate.ToString(), s.CreateAt.ToString("yyyy-MM-dd"))).ToList(),
  ...
  public virtual async Task<BookingResult?> GetBookingByBookingId(Guid bookingId)
  {
   ...
   return new BookingResult(booking.Id, booking.Status, booking.TotalPrice, booking.Guest.CountryFlag, booking.Guest.Nationality);
  }
